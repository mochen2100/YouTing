...
public class HttpParser
{
	...
	public byte[] getRequestBody(byte[] source, int length)
	{
		List<byte[]> httpRequest = getHttpBody(HTTP_REQUEST_BEGIN,
				HTTP_BODY_END, source, length);
		if (httpRequest.size() > 0) { return httpRequest.get(0); }
		return null;
	}
	...
	public ProxyRequest getProxyRequest(byte[] bodyBytes, long urlsize)
	{
		...
		if (remotePort == -1) result._body = result._body.replace(":"
				+ localPort, "");
		else result._body = result._body.replace(":" + localPort, ":"
				+ remotePort);
		...
		} else
		{
			...
			try
			{
				...
				{
					result._body = result._body.replaceAll(RANGE_PARAMS
							+ rangePosition, RANGE_PARAMS + "0");
					result._rangePosition = 0;
	...
	public ProxyResponse getProxyResponse(byte[] source, int length)
	{
		List<byte[]> httpResponse = getHttpBody(HTTP_RESPONSE_BEGIN,
				HTTP_BODY_END, source, length);
		if (httpResponse.size() == 0) return null;
		...
		Log.i(TAG + "<---", text);
		// Get the binary data
		if (httpResponse.size() == 2) result._other = httpResponse.get(1);
		...
		try
		{
			...
			if (text.contains(CONTENT_RANGE_PARAMS) == false)
			{
				...
				{
					String duration = Utils.getSubString(text,
	...
	private List<byte[]> getHttpBody(String beginStr, String endStr,
			byte[] source, int length)
	{
		...
		if (responseStr.contains(beginStr) && responseStr.contains(endStr))
		{
			...
			if ((headerBufferLength > header.length)
					&& (responseStr.indexOf(HTTP_DOCUMENT_BEGIN, header.length) == -1))
			{
