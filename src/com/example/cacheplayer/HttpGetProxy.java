...
import com.example.cacheplayer.HttpParser.ProxyRequest;
import com.example.cacheplayer.HttpParser.ProxyResponse;
...
public class HttpGetProxy
{
	...
	public String getLocalURL()
	{
		...
		} else
		{
			...
			Log.i(TAG+"getLocalURL",remoteHost+"绌烘牸"+remotePath);
			...
			{
				ftpenable = false;
				Log.i(TAG+"getLocalURL",localUrl);
			}
			return localUrl;
		}
	}
	...
	private class Proxy
	{
		...
		public void run()
		{
				...
				try
				{
					...
					sckUser.getOutputStream()
							.write(("$MyNick "
									...
									+ Hub
									+ "|$Supports MiniSlots ADCGet TTHL TTHF|$Direction Download 7777|$Key 鐗�鐩�A 椹穿锟斤拷 0 0 0 0 0|"
									+ "$ADCGET file TTH/" + TTH + " "
					...
					|| (useDC == false))
			{
				...
				Log.v(TAG,"lusyn:Read from HTTP Internet");
				try
				{
					if ((request != null) && (isExists == false)
							&& (mMediaFilePath == newPath))
					{	
						Log.v(TAG,"request not null,isExits false new Path");
						try
						{
						...
						sckPlayer.setSoTimeout(2500);
						sckServer.setSoTimeout(2500); // without this flac not work.
						error = false;
						os = new RandomAccessFile(file1, "rwd");
					} else
					{
						// MediaPlayer's request is invalid
						Log.v(TAG,"MediaPlayer's request is invalid");
						closeSockets();
						return;
					}
					// ------------------------------------------------------
